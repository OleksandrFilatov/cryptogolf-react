...
import { FaTwitter, FaWhatsapp, FaTelegramPlane, FaFacebookF } from "react-icons/fa"
...
const TokenDetail = () => {
    ...
    const getTokenDetail = async () => {
        ...
        setTokenName("CryptoGolf")
        setTokenSymbol("GOLFPUNKS")
    ...
    const getOwnerCollections = async () => {
        ...
        let token_id;
        let token_metadata = [];
        await Promise.all(tokenData.map(async (data) => {
            token_id = data.tokenURI.split("ipfs://")[1].split("/")[1].split(".json")[0];
            const metadata = await axios.get("https://cryptogolf.mypinata.cloud/ipfs/" + data.tokenURI.split("ipfs://")[1])
    ...
    const getAllTokenData = async () => {
        ...
        let etherTokenData = await contract.methods.getSampleTokens().call();
        if (etherTokenData !== '[]') {
        ...
        let bscTokenData = await contract.methods.getSampleTokens().call();
        if (bscTokenData !== "[]") {
    ...
    const handleCancelListing = async () => {
        ...
        if (window.confirm("Are you sure to cancel listing?")) {
            ...
            await contract.methods.removeFromListedPrice(tokenId)
    ...
    const handleCancelMyOffer = async () => {
        const contract = getContract()
        await contract.methods.cancelOffer(walletAddress, tokenId)
            .send({ from: walletAddress })
            .on("receipt", (receipt) => {
    ...
    const handleCancelOffer = async (data) => {
        try {
            ...
            const contract = getContract();
            await contract.methods.cancelOffer(data.buyer, tokenId)
                .send({
                    from: walletAddress
                })
                .on("receipt", (receipt) => {
                            ...
                            <>
                                <Row>
                                    ...
                                    <Col lg="6" xs="12" className="mt-2">
                                        ...
                                        <div className="my-3 color-grey text-left font-small d-flex">
                                            <FacebookShareButton
                                                url={Object.keys(tokenData).length > 0 ? tokenData.image_original_url : ''}
                                                // quote={"フェイスブックはタイトルが付けれるようです"}
                                                // hashtag={"#hashtag"}
                                                description={"GOLFPUNKS is a collection of 10,000 uniquely generated ERC721 token on the blockchain and hosted on IPSF. A total of 5,000 is programmatically generated on the Binance Smart Chain and 5,000 on the Ethereum blockchain."}
                                                className="mx-2"
                                                title={tokenSymbol + " #" + tokenId}
                                            >
                                                <FaFacebookF size={20} title="Share on Facebook" />
                                            </FacebookShareButton>
                                            <TwitterShareButton
                                                url={Object.keys(tokenData).length > 0 ? tokenData.image_original_url : ''}
                                                className="mx-2"
                                                ...
                                                description={"GOLFPUNKS is a collection of 10,000 uniquely generated ERC721 token on the blockchain and hosted on IPSF. A total of 5,000 is programmatically generated on the Binance Smart Chain and 5,000 on the Ethereum blockchain."}
                                            >
                                                <FaTwitter size={20} title="Share on Twitter" />
                                            </TwitterShareButton>
                                            <TelegramShareButton
                                                url={Object.keys(tokenData).length > 0 ? tokenData.image_original_url : ''}
                                                className="mx-2"
                                                ...
                                                description={"GOLFPUNKS is a collection of 10,000 uniquely generated ERC721 token on the blockchain and hosted on IPSF. A total of 5,000 is programmatically generated on the Binance Smart Chain and 5,000 on the Ethereum blockchain."}
                                            >
                                                <FaTelegramPlane size={20} title="Share on Telegram" />
                                            </TelegramShareButton>
                                            <WhatsappShareButton
                                                url={Object.keys(tokenData).length > 0 ? tokenData.image_original_url : ''}
                                                className="mx-2"
                                                ...
                                                description={"GOLFPUNKS is a collection of 10,000 uniquely generated ERC721 token on the blockchain and hosted on IPSF. A total of 5,000 is programmatically generated on the Binance Smart Chain and 5,000 on the Ethereum blockchain."}
                                            >
                                                <FaWhatsapp size={20} title="Share on Whatsapp" />
                                            </WhatsappShareButton>
                                        </div>
                                        {
